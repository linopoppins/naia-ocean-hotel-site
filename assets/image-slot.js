...
(() => {
  ...
  function load() {
    ...
    // Outside the omelette editor there's no sidecar to hydrate and never
    // will be — every slot already carries its final `src`. Skip the fetch
    // so real site visitors don't pay for a request that only ever 404s.
  ...
  class ImageSlot extends HTMLElement {
    static get observedAttributes() {
      return ['shape', 'radius', 'mask', 'fit', 'position', 'placeholder', 'src', 'id', 'alt', 'loading', 'fetchpriority'];
    }
    ...
    _render() {
      ...
      this._img.loading = this.getAttribute('loading') || 'eager';
      this._img.decoding = 'async';
      var fp = this.getAttribute('fetchpriority');
